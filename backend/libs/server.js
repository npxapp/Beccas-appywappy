...
// After ContentService class, add:
// Base Model Layer
// Enhance BaseModel with additional functionality
class BaseModel {
  constructor(dbo) {
    this.dbo = dbo;
    this.tableName = '';
  ...
  // New method to filter fields based on publicFields
  filterFields(data) {
    if (!Array.isArray(this.publicFields) || this.publicFields.length === 0) {
      return data; // Return all fields if no publicFields specified
    }

    // Handle both single objects and arrays
    if (Array.isArray(data)) {
      return data.map(item => this.filterSingleItem(item));
    }
    return this.filterSingleItem(data);
  }

  // Helper method to filter a single item
  ...
  // Modify the existing find method to use filtering
  async find(conditions = {}, options = {}) {
    const result = await this.dbo.find(this.tableName, conditions, options);
    return result;
  }
  
async findWithJoinsNew(conditions = {}, joinConditions = {}, options = {}) {
  const whereClauses = [];
  const values = [];
  let joinClauses = '';

  // Support dynamic joins passed in options.joins (can include aliases)
  if (this.joinTables.length > 0 || options.joins) {
    const allJoins = [
      ...this.joinTables,
      ...(options.joins ? [{ table: '', condition: options.joins }] : [])
    ];

    joinClauses = allJoins
      .map(join => {
        // Ensure that aliasing is correctly handled
        if (join.table && join.condition) {
          return `JOIN ${join.table} ${join.condition}`; // Ensure the alias is used correctly
        }
  ...
  // Build WHERE clauses for conditions, handling aliases in the table name
  const baseTableName = this.tableName.split(' ')[0]; // Remove alias if exists
  Object.entries(conditions).forEach(([key, value]) => {
    whereClauses.push(`${baseTableName}.${key} = $${values.length + 1}`);
    values.push(value);
  });

  // Build WHERE clauses for any additional join conditions
  ...
  // Use the provided fields or fallback to getSelectFields
  const selectFields = options.fields?.join(', ') || this.getSelectFields();
  ...
  // Execute the query
  const result = await this.dbo.find(this.tableName, conditions, { ...options, joins: joinClauses });
  return result;
}
...
async findWithJoins(conditions = {}, joinConditions = {}, options = {}) {
  ...
  // Handle the joins - now expecting a complete JOIN clause in options.joins
  const joinClauses = options.joins || '';

  // Handle table name with alias
  const baseTableName = this.tableName.split(' ')[0]; // Gets 'termrelationships' from 'termrelationships tr'
  const tableAlias = this.tableName.split(' ')[1] || baseTableName; // Gets 'tr' or falls back to table name

  // Build WHERE clauses for conditions using the table alias
  Object.entries(conditions).forEach(([key, value]) => {
    whereClauses.push(`${tableAlias}.${key} = $${values.length + 1}`);
    values.push(value);
  });
  
  // Build WHERE clauses for join conditions
  Object.entries(joinConditions).forEach(([key, value]) => {
    whereClauses.push(`${key} = $${values.length + 1}`);
    values.push(value);
  });
  ...
  // Use the provided fields or get default select fields
  ...
  // Execute the query directly with dbo.execute
  return await this.dbo.execute(query, values);
}

  // Add filtering to other data retrieval methods
  async getById(id) {
    const result = await this.dbo.find(this.tableName, { id });
    return this.filterFields(result[0]);
  }

  async getAll(options = {}) {
    const result = await this.dbo.find(this.tableName, {}, options);
    return this.filterFields(result);
  }
...
class LinkModel extends BaseModel {
  ...
  async getByRel(rel) {
    ...
    return this.filterFields(result); // Now only returns the specified public fields
  }

  async searchLinks(searchTerm) {
    const searchPattern = `%${searchTerm}%`;
    return await this.dbo.findAll(
      this.tableName,
      {
        $or: [
...
// Post Model with enhanced querying capabilities and field filtering
...
// Enhanced Term Model with relationships and filtering
class TermModel extends BaseModel {
  constructor(dbo) {
    super(dbo);
    this.tableName = 'terms';
    this.publicFields = ['term_id', 'name', 'slug', 'term_group'];
...
async getTermRelationships() {
  // Set up the join conditions with aliases
  this.tableName = 'termrelationships tr'; // Alias for termrelationships

  const result = await this.findWithJoins(
    {}, // No specific conditions
    {}, // No specific join conditions
    {
      fields: [
        'tr.object_id',
        'tr.term_taxonomy_id',
...
async getTermRelationshipsNew() {
  // Set up the join conditions dynamically with table and alias pairs
  this.tableName = 'termrelationships tr'; // Alias for termrelationships
  this.joinTables = [
    { table: 'termtaxonomy tt', condition: 'ON tr.term_taxonomy_id = tt.term_taxonomy_id' },
    { table: 'terms t', condition: 'ON tt.term_id = t.term_id' },
    { table: 'posts p', condition: 'ON tr.object_id = p.id' }
  ];
...
// Structure Model with filtering
...
const startServer = async () => {
  try {
    ...
    // 2. Final version of models constant
    const models = {
      links: new LinkModel(dbo),
      posts: new PostModel(dbo),
      terms: new TermModel(dbo),
      taxonomy: new TaxonomyModel(dbo),
      structure: new StructureModel(dbo)
    };
...
app.get('/api/structureNew/tables', authMiddleware, async (req, res) => {
  try {
    const tables = await models.structure.getTables();
    res.json(tables);
  } catch (error) {
    console.error('Error fetching tables:', error);
    res.status(500).json({ 
...
app.get('/api/structureNew/:tableName', authMiddleware, async (req, res) => {
  try {
    const { tableName } = req.params;
    const columns = await models.structure.getTableColumns(tableName);
    ...
    app.get('/api/posts/:postType?', async (req, res) => {
      try {
        const postType = req.params.postType || 'post';
        const posts = await models.posts.getByType(postType);
        res.status(200).json({
          status: 'OK',
          posts,
        });
    ...
    app.get('/api/terms', async (req, res) => {
      try {
        const terms = await models.terms.getTermsWithTaxonomy();
        res.status(200).json({
          status: 'OK',
          terms,
        });
    ...
    app.get('/api/taxonomy', async (req, res) => {
      try {
        const taxonomies = await models.taxonomy.getAllTaxonomies();
        res.status(200).json({
          status: 'OK',
          taxonomies,
        });
    ...
    app.get('/api/termrelationships', async (req, res) => {
      try {
        const relationships = await models.terms.getTermRelationships();
        res.status(200).json({
          status: 'OK',
          relationships
        });
...
app.get('/api/links/:rel', async (req, res) => {
  try {
    ...
    const links = await models.links.getByRel(rel);
    res.status(200).json({
      status: 'OK',
      links
    });
    ...
    app.get('/api/links/searchOld/:searchTerm', async (req, res) => {
      try {
        ...
        console.error(`Error fetching ${req.params.searchTerm} links:`, error);
        res.status(500).json({
    ...
    app.get('/api/links/search/:searchTerm', async (req, res) => {
      try {
        const { searchTerm } = req.params;
        const links = await models.links.searchLinks(searchTerm);
    ...
    app.get('/api/links/footer', async (req, res) => {
      try {
        const links = await models.links.getByRel('footer');
        res.status(200).json({
          status: 'OK',
          links,
        });
    ...
    app.get('/api/links/mobile', async (req, res) => {
      try {
        const links = await models.links.getByRel('mobile');
        res.status(200).json({
          status: 'OK',
          links,
        });
    ...
    app.get('/api/links/dashboard', async (req, res) => {
      try {
        const links = await models.links.getByRel('dashboard');
        res.status(200).json({
          status: 'OK',
          links,
        });
    ...
    app.get('/api/public/links/rel/:rel', async (req, res) => {
      try {
        const { rel } = req.params;
        const results = await models.links.getByRel(rel);
        res.json({ status: 'OK', results });
      } catch (error) {
        res.status(500).json({ error: error.message });
      }
    ...
    app.get('/api/public/posts/type/:type', async (req, res) => {
      try {
        ...
        const results = await models.posts.getPublishedByType(type);
        res.json({ status: 'OK', results });
      } catch (error) {
        res.status(500).json({ error: error.message });
      }
