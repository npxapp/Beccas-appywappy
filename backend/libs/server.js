...
const startServer = async () => {
  try {
    ...
    app.post('/api/auth/login', async (req, res) => {
      ...
      app.get('/api/auth/person', authMiddleware, async (req, res) => {
        try {
          const userData = await controllers.people.getCurrentUser(req.user.id);
          res.json(userData);
        } catch (error) {
          res.status(500).json({ 
      ...
      // Update user profile
      app.put('/api/auth/person', authMiddleware, async (req, res) => {
        try {
          const updatedUser = await controllers.people.updateProfile(req.user.id, req.body);
      ...
      app.get('/api/auth/people/search', authMiddleware, async (req, res) => {
        try {
          ...
          const users = await controllers.people.searchUsers(term);
          res.json(users);
        } catch (error) {
          res.status(500).json({ 
