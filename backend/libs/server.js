...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    /*app.put('/api/content/:tableName/:rowId/field/:columnName', authMiddleware, async (req, res) => {
      try {
        const { tableName, rowId, columnName } = req.params;
  ...
  try {
    ...
    app.post('/api/structure/:tableName', authMiddleware, async (req, res) => {
      try {
        ...
        await dbo.createTable(tableName, columns);
  ...
  try {
    ...
    app.get('/api/structure/:tableName', authMiddleware, async (req, res) => {
      try {
        ...
        `;
        const columns = await dbo.execute(query, [tableName]);
        res.json({ columns });
      } catch (error) {
        res.status(500).json({ error: error.message });
      }
    ...
    app.put('/api/structure/:tableName/columns/:columnName', authMiddleware, async (req, res) => {
      try {
        const { tableName, columnName } = req.params;
        const updatedColumn = req.body;
    
        if (!updatedColumn) {
          return res.status(400).json({ error: 'Updated column data is required' });
        }
    
        // Ensure the updated column contains necessary data
        if (!updatedColumn.name || !updatedColumn.type) {
          return res.status(400).json({ error: 'Column name and type are required' });
        }
    
        const query = `
          ALTER TABLE ${tableName}
          ALTER COLUMN ${columnName} SET DATA TYPE ${updatedColumn.type}
          ${updatedColumn.nullable ? 'SET NOT NULL' : 'DROP NOT NULL'}
        `;
        
        await dbo.execute(query);
        res.status(200).json({ message: `Column "${columnName}" in table "${tableName}" updated successfully.` });
      } catch (error) {
        res.status(500).json({ error: error.message });
      }
    });
