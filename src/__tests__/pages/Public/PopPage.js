...
import { useDynamicPagesContent } from 'pages/Utils/DynamicPagesContent';

import '../App.css';
import '../Fonts.css';

const PopPage = ({ pageName }) => {
  // Pro page specific state
  ...
  // Filter cards based on the passed pageName prop
  const cards = allCards.filter(card => card.slug === pageName);
  const [openCards, setOpenCards] = useState({});

  // Pro page specific effects and handlers
  useEffect(() => {
    if (pageName === 'Pro') {
      setDrawerOpen(true);
    }
      ...
      case 'post':
        return (
          <Card key={cardId}>
            <CardHeader
              title={card.slug}
              action={renderIcon(card.icon || card.asset_status)}
            />
            <CardContent>
              <Button
                variant="outlined"
