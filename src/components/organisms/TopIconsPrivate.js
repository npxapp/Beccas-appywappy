...
import { useMenu } from 'contexts/MenuContext'; // Changed from useCollapse
...
const TopIconsPrivate = ({ isXs, isHomePage, useTheme: propsUseTheme, user }) => {
  ...
  const { isMenuOpen, toggleMenu } = useMenu(); // Changed from useCollapse
  ...
  const handlePortalClick = () => {
    // Use the toggleMenu from context
    toggleMenu();
    
    // Toggle sticky bar
    if (!showStickyBar) {
      toggleStickyBar();
    } else if (!isMenuOpen) { // If closing menu, hide sticky bar
      toggleStickyBar();
    }
  };
  ...
  const renderToolbarContent = () => (
    <>
      ...
      >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          ...
          <IconButton
            color="primary"
            aria-label="menu"
            onClick={toggleMenu} // Changed from toggleCollapse
      ...
      <MobileDrawer 
        in={isMenuOpen} // Changed from isCollapseOpen 
        toggleCollapseMenu={toggleMenu} // Changed from toggleCollapse
        setMenuOpen={() => {}} // No longer needed since we're using context
        width="100%"
        zIndex={1}
        color="text.primary"
      />
