...
import { useMenu } from 'contexts/MenuContext'; // Updated from useCollapse
...
const TopIcons = ({ isXs, isHomePage, useTheme: propsUseTheme }) => {
  ...
  const { isMenuOpen, toggleMenu } = useMenu(); // Updated from useCollapse
  const { showStickyBar, toggleStickyBar } = useAppBar();
  ...
  const handlePortalClick = () => {
    // Use the toggleMenu from context, which will handle all the menu state
    toggleMenu();
  ...
  const renderToolbarContent = () => (
    <>
      ...
      <MobileDrawer 
        ...
        setMenuOpen={() => {}} // This is no longer needed as we're using context state
        width="100%"
        zIndex={1}
        color="text.primary"
      />
